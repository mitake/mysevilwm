...
void keys_init() {
    ...
    for (i = 0; i < key_len; i++) {
        ...
        keys[i].ev = keys_ev[i];
        keys[i].arg = (char*)calloc(sizeof(char), strlen(keys_args[i]) + 1);
...
int keys_run(XKeyEvent* e) {
    extern const unsigned int sevilwm_key_event_modifiers;
    int k = XKeycodeToKeysym(dpy, e->keycode, 0);
    if (e->state & ShiftMask) k |= SHIFT;
    if (!(sevilwm_key_event_modifiers & ControlMask) &&
        e->state & ControlMask) k |= CTRL;
    if (!(sevilwm_key_event_modifiers & Mod1Mask) &&
        e->state & Mod1Mask) k |= ALT;
...
void keys_run_button(XButtonEvent* be) {
    ...
    else if (be->button == Button4) k = XK_Pointer_Button4;
    else if (be->button == Button5) k = XK_Pointer_Button5;
    if (be->state & ShiftMask) k |= SHIFT;
    if (be->state & ControlMask) k |= CTRL;
    if (be->state & Mod1Mask) k |= ALT;
    keys_run_impl(k);
}
